...
with nb6:
    if st.session_state.logged_in:
        ...
        if st.button("Log In / Sign Up", key="nb_l", use_container_width=True):
            st.session_state.page = "account"; st.rerun()
with nb7:
    if st.session_state.logged_in:
...
def render_full_analysis(data, show_downloads=True, key_prefix="main"):
    ...
    with sa2:
        if st.session_state.logged_in and st.session_state.is_pro and st.session_state.user_id:
            ...
            if st.button("Upgrade to Pro", key=f"share_up_{key_prefix}", use_container_width=True):
                st.session_state.page = "pricing"; st.rerun()
        else:
    ...
    if st.session_state.share_id_display:
        ...
        <div style="background:rgba(0,212,170,0.08);border:1px solid rgba(0,212,170,0.25);
          ...
          <div style="color:#00D4AA;font-weight:600;font-size:0.88rem;margin-bottom:0.3rem;">{'Share link auto-generated' if st.session_state.logged_in and st.session_state.is_pro else 'Share link created'}</div>
          <div style="color:#C8D0E8;font-size:0.85rem;">Share ID: <b style="font-size:1.1rem;letter-spacing:2px;">{sid}</b></div>
          <div style="color:#8B9BC8;font-size:0.8rem;margin-top:0.3rem;">Go to the "Shared" page and enter this ID to view the report. Requires a Pro account to view.</div>
        </div>""", unsafe_allow_html=True)
    ...
    if not st.session_state.logged_in:
        ...
        with d2:
            if st.session_state.is_pro:
                ...
                if st.button("Upgrade to Pro", key=f"uppdf_{key_prefix}", use_container_width=True):
                    st.session_state.page = "pricing"; st.rerun()
        with d3:
            if st.session_state.is_pro:
...
if st.session_state.page in ("login", "signup", "account"):
    # initialise which tab is active
    if "auth_tab" not in st.session_state:
        st.session_state.auth_tab = "signup" if st.session_state.page == "signup" else "login"
    # if we arrived via old page names, sync the tab
    ...
    with ac:
        ...
        # ── Tab toggle ────────────────────────────────────────────────────────
        tab_login_active  = st.session_state.auth_tab == "login"
        tab_signup_active = st.session_state.auth_tab == "signup"
        def tab_style(active):
            if active:
                return "background:linear-gradient(135deg,#4F8EF7,#00D4AA);color:#080C14;font-weight:700;border:none;"
            return "background:rgba(255,255,255,0.04);color:#8B9BC8;border:1px solid rgba(255,255,255,0.08);"
        ...
        if st.session_state.auth_tab == "login":
            ...
            st.markdown("<div style='height:0.5rem'></div>", unsafe_allow_html=True)
            st.markdown("<div style='text-align:center;color:#4A5578;font-size:0.83rem;'>Don't have an account? <span style='color:#4F8EF7;cursor:pointer;'>Switch to Sign Up above.</span></div>", unsafe_allow_html=True)
            ...
            if st.button("Create Account", key="signup_submit", use_container_width=True):
                if not email or not pw:
                    ...
                    if res["ok"]:
                        ...
                        st.error(res["error"])
            st.markdown("<div style='height:0.5rem'></div>", unsafe_allow_html=True)
            st.markdown("<div style='text-align:center;color:#4A5578;font-size:0.83rem;'>Already have an account? <span style='color:#4F8EF7;cursor:pointer;'>Switch to Log In above.</span></div>", unsafe_allow_html=True)
            if "Pro" in plan:
                st.markdown("""
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if go:
        ...
        # Auto-generate share ID for Pro users
        if st.session_state.logged_in and st.session_state.is_pro and st.session_state.user_id:
            auto_sid = create_share(st.session_state.user_id, data)
