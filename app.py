...
def build_pdf(data: dict) -> bytes:
    ...
    story.append(Paragraph("FINANCIAL STATEMENT ANALYSIS",
                            S("ti", fontSize=22, fontName="Helvetica-Bold",
    ...
    story.append(Paragraph(f"Financial Health: {lbl}  |  Score: {score} / 10",
                            S("hs", fontSize=12, fontName="Helvetica-Bold",
                              textColor=hcol, spaceAfter=12)))
    story.append(div)
    story.append(Paragraph("EXECUTIVE SUMMARY", S("lb", fontSize=7, fontName="Helvetica-Bold",
                                                    textColor=C_MUT, spaceAfter=4)))
    story.append(Paragraph(data.get("health_summary", ""),
                            S("bo", fontSize=10, textColor=C_SUB, leading=15, spaceAfter=8)))
    story.append(Paragraph("INVESTOR VIEW", S("lb2", fontSize=7, fontName="Helvetica-Bold",
                                               textColor=C_MUT, spaceAfter=4)))
    story.append(Paragraph(data.get("investor_view", ""),
                            S("iv", fontSize=10, textColor=C_SUB, leading=15, spaceAfter=8)))
    story.append(div)
 
    # KPI table
    story.append(Paragraph("KEY FINANCIAL METRICS",
                            S("h2", fontSize=13, fontName="Helvetica-Bold",
                              textColor=C_BLUE, spaceBefore=6, spaceAfter=6)))
    kpi_keys = [("revenue","Revenue"),("net_profit","Net Profit"),("gross_margin","Gross Margin"),
                ("net_margin","Net Margin"),("ebitda","EBITDA"),
                ("operating_cashflow","Operating Cash Flow"),("current_ratio","Current Ratio"),
                ("debt_to_equity","Debt / Equity"),("working_capital","Working Capital"),
    ...
    for sec_key, sec_title, accent in [
        ...
        story.append(Paragraph(sec_title.upper(),
                                S(f"sh{sec_key}", fontSize=13, fontName="Helvetica-Bold",
                                  textColor=accent, spaceBefore=10, spaceAfter=4)))
        story.append(Paragraph(sec.get("headline",""),
                                S(f"hd{sec_key}", fontSize=10, textColor=C_SUB, leading=15, spaceAfter=4)))
        for pt in sec.get("points",[]):
            story.append(Paragraph(f"• {pt}", S(f"pt{sec_key}", fontSize=10, textColor=C_SUB,
                                                  leading=14, spaceAfter=3)))
        story.append(Spacer(1,4))
    story.append(div)
 
    story.append(Paragraph("KEY RISKS & CONCERNS",
                            S("rh", fontSize=13, fontName="Helvetica-Bold",
                              textColor=C_RED, spaceBefore=6, spaceAfter=6)))
    for i, risk in enumerate(data.get("risks",[])):
        story.append(Paragraph(f"<b>{risk.get('title','')}</b>",
                                S(f"rt{i}", fontSize=10, textColor=C_RED, spaceAfter=2)))
        story.append(Paragraph(f"Issue: {risk.get('detail','')}",
                                S(f"rd{i}", fontSize=10, textColor=C_SUB, leading=14, spaceAfter=2)))
        story.append(Paragraph(f"Action: {risk.get('fix','')}",
                                S(f"rf{i}", fontSize=10, textColor=C_TEAL, leading=14, spaceAfter=6)))
 
    story.append(Paragraph("POSITIVE SIGNALS",
                            S("ph", fontSize=13, fontName="Helvetica-Bold",
                              textColor=C_TEAL, spaceBefore=6, spaceAfter=6)))
    for i, pos in enumerate(data.get("positives",[])):
        story.append(Paragraph(f"<b>{pos.get('title','')}</b>",
                                S(f"pos{i}", fontSize=10, textColor=C_TEAL, spaceAfter=2)))
        story.append(Paragraph(pos.get("detail",""),
                                S(f"posd{i}", fontSize=10, textColor=C_SUB, leading=14, spaceAfter=6)))
 
    story.append(Paragraph("RECOMMENDATIONS",
                            S("rch", fontSize=13, fontName="Helvetica-Bold",
                              textColor=C_BLUE, spaceBefore=6, spaceAfter=6)))
    for i, rec in enumerate(data.get("recommendations",[]), 1):
        story.append(Paragraph(f"<b>{i}. {rec.get('action','')}</b>",
                                S(f"rc{i}", fontSize=10, textColor=C_BLUE, spaceAfter=2)))
        story.append(Paragraph(rec.get("rationale",""),
                                S(f"rcd{i}", fontSize=10, textColor=C_SUB, leading=14, spaceAfter=6)))
    story.append(div)
...
DEFAULTS = {"page":"analyser","logged_in":False,"user_email":"","user_id":None,
            ...
            "compare_ids":[],"share_id_display":None}
...
nav_map = [
    (nb1,"Analyser","analyser"),(nb2,"Features","features"),(nb3,"Pricing","pricing"),
    (nb4,"Dashboard","dashboard"),(nb5,"Shared","shared_view"),
]
for col, label, page in nav_map:
    with col:
        if st.button(label, key=f"nb_{page}", use_container_width=True):
            st.session_state.page = page if (st.session_state.logged_in or page not in ["dashboard"]) else "login"
            st.rerun()
with nb6:
    lbl = st.session_state.user_email.split("@")[0] if st.session_state.logged_in else "Log In"
    if st.button(lbl, key="nb_login", use_container_width=True):
...
with nb7:
    if st.session_state.logged_in:
        ...
        if st.button("Sign Up", key="nb_signup", use_container_width=True):
...
# ─────────────────────────────────────────────────────────────────────────────
# ANALYSIS RENDER HELPERS  (reused across analyser + dashboard + shared views)
# ─────────────────────────────────────────────────────────────────────────────
...
def render_full_analysis(data, kp="main", allow_save=True):
    """Render complete analysis. kp=key_prefix to avoid duplicate widget keys."""
    render_banner(data)
    ...
    with sa3:
        if st.button("My Dashboard", key=f"dash_{kp}", use_container_width=True):
            st.session_state.page="dashboard" if st.session_state.logged_in else "login"; st.rerun()
    with sa4:
    ...
    if st.session_state.share_id_display:
        ...
        <div style="background:rgba(0,212,170,0.08);border:1px solid rgba(0,212,170,0.25);
          ...
          <div style="color:#8B9BC8;font-size:0.8rem;margin-top:0.3rem;">Go to "Shared" in the nav bar and enter this ID to view the report.</div>
        </div>""", unsafe_allow_html=True)
    ...
    if not st.session_state.logged_in:
        ...
        <div style="background:linear-gradient(135deg,rgba(79,142,247,0.08),rgba(0,212,170,0.05));
          ...
          <div style="color:#8B9BC8;font-size:0.88rem;">Free: TXT + PDF. Pro: also Excel and Word.</div>
        </div>""", unsafe_allow_html=True)
        ...
        with d2:
            if PDF_OK:
                ...
                st.markdown("<div style='background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:0.55rem;text-align:center;color:#4A5578;font-size:0.82rem;'>PDF unavailable</div>",unsafe_allow_html=True)
        with d3:
...
if st.session_state.page == "login":
    ...
    with lc:
        ...
        <div style="text-align:center;padding:1rem 0 2rem;">
          ...
          <p style="color:#8B9BC8;font-size:0.9rem;margin:0;">Log in to access your reports and dashboard.</p>
        </div>""", unsafe_allow_html=True)
...
# ═════════════════════════════════════════════════════════════════════════════
# SHARED DISPLAY (read-only, no login required)
# ═════════════════════════════════════════════════════════════════════════════
    ...
    <div style="background:rgba(79,142,247,0.08);border:1px solid rgba(79,142,247,0.2);border-radius:10px;
      ...
      <span style="color:#4F8EF7;font-size:0.85rem;">Read-only shared report — no login required to view.</span>
    </div>""", unsafe_allow_html=True)
    D()
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    for col,(num,title,desc) in zip([s1,s2,s3,s4],[
        ...
        ("04","Review and download","Free: TXT + PDF. Pro: also Excel and Word. Save to dashboard and share links.")]):
        with col:
            st.markdown(f"""
            <div style="background:linear-gradient(135deg,#0F1620,#111827);border:1px solid rgba(255,255,255,0.07);
